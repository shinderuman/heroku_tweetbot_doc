	...
	EYE.extend({
		...
		spacegallery: {
			...
			defaults: {
				...
				perspective: 400, // perpective height
			...
			//position images
			positionImages: function(el,spacegallery) {
				var top = 0;
				EYE.spacegallery.animated = false;
				center = parseInt($(document).width() / 2);
				$(el)
					.find('a')
					...
					.find('div')
						.removeAttr('height')
						.each(function(nr){
							var newWidth = spacegallery.origWidth - (spacegallery.origWidth - spacegallery.origWidth * el.spacegalleryCfg.minScale) * el.spacegalleryCfg.asins[nr];
							$(this)
								.css({
									top: el.spacegalleryCfg.tops[nr] + 'px',
									marginLeft: center - parseInt((newWidth + el.spacegalleryCfg.border)/2, 10) + 'px',
									opacity: 1 - (el.spacegalleryCfg.asins[nr] * 2),
									width: parseInt(newWidth) +'px',
									height: parseInt(newWidth * 0.75)	 +'px'								
								})
								.find('h1')
								  .css({
							        top: parseInt(100 *(1 - el.spacegalleryCfg.asins[nr])) + 'px',
								      fontSize: parseInt(34 *(1 - el.spacegalleryCfg.asins[nr])) + 'px'
								  })
								.end()
							if (!this.spacegallery)this.spacegallery={};
							...
							this.spacegallery.opacity = 1 - (el.spacegalleryCfg.asins[nr] * 2);
							...
							this.spacegallery.origWidth = spacegallery.origWidth;
						...
						.animate({
							...
							complete: function() {
								$(el)
									...
									.prependTo(el)
									.end()
								EYE.spacegallery.positionImages(el,el.spacegallery);
								el.spacegalleryCfg.after.apply(el);
								$(el).find('div:last')
								.each(function(n,front){$('#created_at').empty().append(prettyDate($(front).attr('created_at')));})
							},
							step: function(now) {
								$('div', this)
									.each(function(nr){
      								...
      								.find('h1')
      								  .css({
      								      fontSize: parseInt(34 * ratio) +2 + 'px'
      								  })
      								.end()										
										}
									});
			...
			init: function(opt) {
				...
				return this.each(function(){
					...
					if ($(el).is('.spacegallery')) {
						...
						el.spacegalleryCfg.images = el.getElementsByTagName('div').length;
						...
						$('div', el)
							.each(function(nr){
								...
								el.spacegallery = {};
								el.spacegallery.origWidth = 700;
								el.spacegallery.origHeight = 300;
								EYE.spacegallery.positionImages(el,el.spacegallery);
